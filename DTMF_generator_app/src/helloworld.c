...
static void Timer_ISR( void *CallBackRef){
	...
	switch (dtmf_state){
	case PLAY_TONE: {
        Generate_DTMF(dtmfSequence[sequenceIndex]);
        toneTimer--;
        if (toneTimer <= 0) {
            dtmf_state = PAUSE;
